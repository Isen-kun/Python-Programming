...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    with open(gdpinfo['gdpfile'], 'r') as data_file:
        data = csv.DictReader(
            data_file, delimiter=gdpinfo['separator'], quotechar=gdpinfo['quote'])
        for row in data:
            new_data_dict[row[gdpinfo['country_name']]] = row

    plot_dict, plot_set_1 = reconcile_countries_by_name(
        plot_countries, new_data_dict)
...
def render_world_map(gdpinfo, plot_countries, year, map_file):
    ...
    """
    plot_dict_1, plot_set_1, plot_set_2 = build_map_dict_by_name(
        gdpinfo, plot_countries, year)
    worldmap_chart = pygal.maps.world.World()
    title_map = 'GDP by country for ' + year + \
        ' (log scale), unifiedby common country NAME'
